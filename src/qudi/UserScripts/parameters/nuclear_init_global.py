...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            pi_dur_L = self.queue.tt.rp('e_rabi_ou350deg-90-L', omega=_I_['omega']).pi
            amp_L = self.queue.tt.rp('e_rabi_ou350deg-90-L', omega=_I_['omega']).amp

            pi_dur_R = self.queue.tt.rp('e_rabi_ou350deg-90-R', omega=_I_['omega']).pi
            amp_R = self.queue.tt.rp('e_rabi_ou350deg-90-R', omega=_I_['omega']).amp
            mcas.asc(
                A1 = _I_['init']=='A1',
                A2 = _I_['init']=='A2', 
                ...
                pd2g1 = {
                    ...
                    'frequencies':[177.7,169.7]
                })  # Init system with A2 laser
            ...
            sna.electron_rabi(
                mcas,
                new_segment=False,
                length_mus= 0.48*_I_['RF_factor'],
                amplitudes=[_I_['RF_PI_amp']/_I_['RF_factor']],
                frequencies=[14.25],
                mixer_deg=[-90]
            )

            l = {'L':pi_dur_L, 'R':pi_dur_R}[_I_['trans']]
            amp = {'L':amp_L, 'R':amp_R}[_I_['trans']]
            sna.electron_rabi(
                mcas,
                new_segment=False,
                length_mus= l,
                amplitudes=[amp],
                frequencies=[_I_['mw_freq']+{'L':34,'R':173}[_I_['trans']]],
                mixer_deg=[-90]
            )
            ...
            if _I_['readout'] == 'A2':
                sna.ssr(mcas = mcas, queue=self.queue, frequencies=freq, wait_dur=0.0, robust=False,
                    nuc='ple_A2', mixer_deg=-90, eom_ampl=0.0, step_idx=0, laser_dur=2.0)
            elif _I_['readout'] == 'A1':
                sna.ssr(mcas = mcas, queue=self.queue, frequencies=freq, wait_dur=0.0, robust=False,
                    nuc='ple_A1', mixer_deg=-90, eom_ampl=0.0, step_idx=0, laser_dur=2.0)
                
            mcas.asc(length_mus=30.0, name='sequence wait 2', A2= True, A1 =True)
            mcas.asc(length_mus=10.0, name='sequence wait 2')
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict( # WHAT DOES ALL THIS MEAN ??? WHICH UNITS ??
        (
            ...
            ('init_time', [100]),
            ('mw_init',[0.02]),
            ('RF_factor',[1.0,2.0,4.0]),
            ('RF_PI_amp',[0.18,0.0]),
            #('mw_freq', np.linspace(-5,5,30)), 
            ('init', ['A1']),
            ('readout',['A1']),
            ('sweeps', range(40)),
            ('trans',['L']),
            ('mw_freq', np.linspace(-7,7,50)), 
        )
    )
...
def run_fun(abort, **kwargs):
    ...
    nuclear.queue._gated_counter.readout_duration = 10*1e6 # --> nvalues.
    nuclear.hashed = True
