...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            mcas.asc(length_mus=3.0, name='sequence wait 1')
            # get the electron out of the -1/2 subspace
            factors=_I_['first pi factor']
            sna.electron_rabi(
                mcas,
                new_segment=False,
                length_mus= 1.23,
                amplitudes=[0.0101616*factors,0.0101616*factors],
                frequencies=[80.6,78.4],
                mixer_deg=[-90,-90]
            )
            ...
            sna.electron_rabi(
                ...
                new_segment=False,
                length_mus= 1.23,
                amplitudes=[0.0101616]*2,
                frequencies=[_I_['mw_freq'],_I_['mw_freq']+1.8],
                mixer_deg=[-90]*2
            )
            mcas.asc(length_mus=0.5, name='sequence wait 1')
            
            #get the electron out of the 1/2 space
            sna.electron_rabi(
                mcas,
                new_segment=False,
                length_mus= 1.3,
                amplitudes=[0.0101616,0.0101616],
                frequencies=[219.9,217.7],
                mixer_deg=[-90,-90]
            )
            mcas.asc(length_mus=0.5, name='sequence wait 1')
            ...
            sna.ssr(mcas = mcas, queue=self.queue, frequencies=freq, wait_dur=0.0, robust=False,
                    nuc=['ple_A2','ple_A1'][_I_['init'] == 'A1'], mixer_deg=-90, eom_ampl=0.0, step_idx=0, laser_dur=30.0)
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict( # WHAT DOES ALL THIS MEAN ??? WHICH UNITS ??
        (
            ...
            ('sweeps', range(10)),
            ('first pi factor', [1,0]),
            ('mw_freq', np.linspace(65,95,200)+1*70), 
            ('init', ['A2']),
        )
    )
