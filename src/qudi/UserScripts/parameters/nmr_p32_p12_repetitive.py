...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            pi_dur = self.queue.tt.rp('e_rabi_ou350deg-90-L1', omega=_I_['omega']).pi
            amp = self.queue.tt.rp('e_rabi_ou350deg-90-L1', omega=_I_['omega']).amp
            mcas.asc(
                A1=_I_['init']=='A1',
                A2 =_I_['init']=='A2', 
            ...
            #freq = [30.0]
            amp  = self.queue.tt.rp('e_rabi_ou350deg-90-L1', omega=_I_['omega']).amp
            amp2  = self.queue.tt.rp('e_rabi_ou350deg-90-R1', omega=_I_['omega']).amp
            # sna.ssr(mcas = mcas, queue=self.queue, frequencies=[[38.5,177.7]], wait_dur=0.0, robust=False,
                    # nuc='29Si8_A2', mixer_deg=-90, eom_ampl=0.0, 
                    # step_idx=0, laser_dur=5.0,length_mus_mw = 0.5, amplitudes=[[amp,amp2]], repetitions = 10
                    # )

            sna.ssr(mcas = mcas, queue=self.queue, frequencies=[[30.4, 168.4]], wait_dur=0.0, robust=False,
                    nuc='29Si8_A2', mixer_deg=-90, eom_ampl=0.0, 
                    step_idx=0, laser_dur=5.0, length_mus_mw = pi_dur, amplitudes=[amp,amp2], repetitions = _I_['n_ssr']
                    )
                    ## Will this work?
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict( # WHAT DOES ALL THIS MEAN ??? WHICH UNITS ??
        (
            ...
            ('sweeps', range(20)),
            ('n_ssr', [10,5,25,50,1]),
            ('mw_freq',[14.25]),#np.linspace(12,18,50)),
            ('tau_rabi',E.round_length_mus_full_sample(np.linspace(0.0, 120.0, 200))), 
            ('init', ['A1']),
            #('cnot_freq',[30.50,38.45]),
            ('readout_phase',[0,180])
        )
    )
...
def run_fun(abort, **kwargs):
    ...
    nuclear.queue._gated_counter.readout_duration = 10e6 # --> nvalues.
