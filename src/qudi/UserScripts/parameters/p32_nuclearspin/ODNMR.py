...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict(
        (
            ('sweeps', range(50)),
            ('A2_power',[5]),
            ('omega', [1/1.024]), # for projective electron pi 
            ('init_state', ['m12']),
            ('init_time', [100]),
            ('readout_dur', E.round_length_mus_to_x_multiple_ps([1.024])),
            ('amp', [0.025]), 
            ('readout', ['A2']),
            #('tau_rabi',E.round_length_mus_full_sample(np.linspace(0.0, 10.0, 50))), 
            ('tau_rabi',E.round_length_mus_full_sample([8.84])), 
            ('mw_freq',np.linspace(12.5,13.2,50)), #[12.53]
            ('cnot_freq',[168.93, 178.35]),
        )
    )
...
def run_fun(abort, **kwargs):
    ...
    nuclear.pld.fit_function = 'lorentz'
