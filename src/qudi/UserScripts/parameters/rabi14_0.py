...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            mcas.asc(length_mus=0.5, A2 = True, name='sequence wait 1')
...
def settings(pdc={}):
    ...
    nuclear.do_confocal_A2MW_refocus = False ## Not implemented with the AWG markerings.
    ...
    nuclear.parameters = OrderedDict( # WHAT DOES ALL THIS MEAN ??? WHICH UNITS ??
        (
            ...
            #('RF amp', [0.1]),
            #('mw_freq',[14.40]),#np.linspace(12,18,50)),
            ('mw_freq',[6.47]),#np.linspace(12,18,50)),
            #('mw_freq', np.linspace(6,7,20)),
            ('tau_rabi',E.round_length_mus_full_sample(np.linspace(0.0, 10.0, 41))), 
            #('tau_rabi',E.round_length_mus_full_sample([2.6])), 
            ('init', ['A2']),
            ('A2_power', [4]), # Is not yet implemented...
            ('cnot_freq',[30.4,38.45])
        )
    )
    nuclear.number_of_simultaneous_measurements =  2#2*len(nuclear.parameters['tau_rabi'])
