...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            amp2  = self.queue.tt.rp('e_rabi_ou350deg-90-R2', omega=_I_['omega']).amp


            sna.ssr(mcas = mcas, queue=self.queue, frequencies=[[38.4, 177.72]], wait_dur=0.0, robust=False,
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict( # WHAT DOES ALL THIS MEAN ??? WHICH UNITS ??
        (
            ...
            ('sweeps', range(20)),
            ('mw_freq',[14.25]),
            ('tau_rabi',E.round_length_mus_full_sample(np.linspace(0.0, 20.0, 51))), 
            ('init', ['A1']),
            ('n_ssr', [1,10,20]),
            ('readout_phase',[0,180])
        )
