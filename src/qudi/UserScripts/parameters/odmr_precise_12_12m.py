...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            pi_dur_R = self.queue.tt.rp('e_rabi_ou350deg-90-R2', omega=_I_['omega']).pi
            ...
            mcas.asc(
                ...
                pd2g1 = {
                    ...
                    'frequencies':[177.78,168.4]
                })  # Init system with A2 laser - into -1/2 up,
            
            mcas.asc(length_mus=10.0, name='sequence wait 1')
            
            ##

            length_pulse = {'pi':1.33, '2pi': 2.66}[_I_['RF']]

            # Flip down or not. ( how else to prepare -1/2 down? ,maybe with +1/2 down?
            sna.electron_rabi(
                mcas,
                new_segment=False,
                length_mus=length_pulse,
                amplitudes=[_I_['RF_PI']],
                frequencies=[6.5],
                mixer_deg=[-90]
            )
            ...
            if _I_['MW -3/2to-1/2']:
                ...
                new_segment=False,
                length_mus= pi_dur_L,
                amplitudes=[amp_L,amp_L],
                frequencies=[38.5,30.5],
                mixer_deg=[-90]*2
                ) # flip -3/2 to -1/2 ?? 
            ...
            amp_c = self.queue.tt.rp('e_rabi_ou350deg-90-C', period = 1.0).amp
            ...
            sna.electron_rabi(
                ...
                length_mus= 0.5,
                amplitudes=[amp_c],
                frequencies=[107,115], #mfl --> gives the frequency , (12-12m, 29Si8+)
                mixer_deg=[-90]*2) # Rabi 1/2-1/2 pi pulse. 
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict( # WHAT DOES ALL THIS MEAN ??? WHICH UNITS ??
        (
            ...
            ('readout', ['A2']),
            ('sweeps', range(30)),
            ('RF_PI',[0.0,0.18]), #flip nuclear or not
            ('MW -3to-1',[False]),
            ('RF',['pi', '2pi']),
            #('mw_duration',E.round_length_mus_full_sample(np.linspace(0.0, 0.3, 61))),
            ('init_time', [100]), # polarize into -1/2
            ('mw_init',[0.02]),
            ('A2_power',[3]),
            ('mw_freq',np.linspace(165,183,60)), 
            #('mw_freq',[108.5]),#np.linspace(90,130,40)), 
            ('init', ['A2']),
        )
    )
