...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            mcas.asc(A1=_I_['init']=='A1',A2 =_I_['init']=='A2', 
                        length_mus=_I_['init_time'], name='resonant_init')  # Init system with A2 laser
            mcas.asc(length_mus=3.0, name='sequence wait 1')
            sna.electron_rabi(
                mcas,
                new_segment=False,
                length_mus= _I_['tau_rabi'],
                amplitudes=[_I_['power']],
                frequencies=[217.55],
                mixer_deg=[-90]
            )
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict( # WHAT DOES ALL THIS MEAN ??? WHICH UNITS ??
        (
            ...
            ('readout', ['A1']),
            ('init_time', [50]),
            ('power', np.linspace(0.01,0.4,10)),
            ('sweeps', range(10)),
            ('tau_rabi', E.round_length_mus_full_sample(np.linspace(0,2.000,100))), 
            ('init', ['A2']),
        )
    )
    nuclear.number_of_simultaneous_measurements =  1*len(nuclear.parameters['tau_rabi'])
