...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            mcas.asc(length_mus=10.0)  # Starting... histogram 0
            ...
            def init_state_drive(state):
                ...
                pd2g1 = {
                    ...
                    'frequencies': [177.72, 168.24, 30.4, 38.4, 3.4, 6.47]
                }
            ...
            mcas.asc(length_mus=1.0, name='waitAfterInit')
            ...
            if _I_['cnot_freq'] == 30.4:
                ...
                amp = self.queue.tt.rp('e_rabi_ou350deg-90-L2', omega=_I_['omega']).amp
            sna.electron_rabi(
                mcas,
                new_segment=False,
                length_mus= pi_dur,
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict(
        (
            ('sweeps', range(50)),
            ('A2_power',[5]),
            ('omega', [1.5]), # for projective electron pi 
            ('init_state', ['m12-']), ##########################m32-
            ('init_time', [100]), ########################[10, 20, 50,100, 150]
            ('readout_dur', [1.05]), ###############################0.05
            ('mw_freq',[3.2]),
            ('amp', [0.05]), 
            ('cnot_freq',[177.72]),
            ('tau_rabi',E.round_length_mus_full_sample(np.linspace(0.0, 7.0, 26))), 
            ('readout', ['A2', 'A1']),
        )
    )
