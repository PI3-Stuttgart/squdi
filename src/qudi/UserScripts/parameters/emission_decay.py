...
import os
import importlib
import notebooks.UserScripts.helpers.sequence_creation_helpers as sch;
...
import notebooks.UserScripts.helpers.shared as ush;
...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            mcas.asc(A1=(_I_['init'] == 'A1'), A2=(_I_['init'] == 'A2'), length_mus=15., name='init')
            mcas.asc(length_mus=10, name='sequence wait 1')
            
            if _I_['readout_laser'] == 'A2':
                sna.ssr(
                    mcas = mcas,
                    queue=self.queue,
                    frequencies=freq,
                    ...
                    step_idx=0,
                    laser_dur=30.0,
                    repetitions = _I_['bins'])


            # for bins in range(0,np.max(_I_['bins'])):
            #     print(bins)
            #     if _I_['readout_laser'] == 'A2':
            #         mcas.asc(length_mus=_I_['readout_binning']/2, A2=True, gate=True, name='gate1') 
            #         mcas.asc(length_mus=_I_['readout_binning']/2, A2=True, gate=False, name='gate2')
            #     # elif _I_['readout_laser'] == 'A1':
            #     #     mcas.asc(length_mus=_I_['readout_binning']/2, A1=True, gate=True, name='gate1')
            #     #     mcas.asc(length_mus=_I_['readout_binning']/2, A1=True, gate=False, name='gate1')

            # mcas.asc(length_mus=1.0, memory = False, name = 'memory1')
            # mcas.asc(length_mus=0.1, memory = True, name = 'memory1')
            # mcas.asc(length_mus=1.0, memory = False , name = 'wait')

        self.queue._gated_counter.set_n_values(mcas,
                                               self.number_of_simultaneous_measurements)  # how to get here the queue? readout duration/sequence length.
...
def settings(pdc={}):
    ...
    nuclear.A1LaserPower = 3 #nW
    nuclear.A2LaserPower = 3 #nW

    nuclear.ple_refocus_interval = 50
    nuclear.confocal_refocus_interval = 50  # seconds
