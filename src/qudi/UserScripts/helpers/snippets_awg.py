...
import importlib
import qudi.hardware.Keysight_AWG_M8190.pym8190a as MCAS; importlib.reload(MCAS)
import qudi.UserScripts.helpers.sequence_creation_helpers as sch; importlib.reload(sch)
...
from qudi.hardware.Keysight_AWG_M8190.elements import WaveFile, WaveStep, SequenceStep, Sequence
import qudi.hardware.Keysight_AWG_M8190.pym8190a as pym8190a
import numbers
    ...
    def set_rp(self, kwargs):
        ...
        if 'rp' in kwargs:
            ...
            self.rp = self.queue.tt.rabi_parameters['e_rabi_ou{:.0f}deg{}-{}'.format(
                350,#self.queue._awg.mcas_dict.awgs['2g'].ch[1].output_amplitude*1000, #TODO here is hardcoded for 350 mV ampl.change to the config option of the AWG to be used.
                self.mixer_deg,
                ms_transition
            )]
        else:
