...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        sequence_name = "RABI_test" if sequence_name is None else sequence_name
            ...
            with infinite_loop_():
                ...
                with for_(*from_array(i_1, i_1_array)):
                    with for_(*from_array(i_2, i_2_array)):
                        ...
                        align()
                        # Play the mw pulse...
                        play("cw" * amp(1), "NV", duration=chk_i("tau") / 4)
                        align()
                        wait(duration=2 * u.us - chk_i("tau") / 4)
                        align()
...
def settings(pdc={}):
    ...
    tau_array = np.arange(24 * u.ns, 400 * u.ns, 1 * u.ns)
    ...
    nuclear.number_of_simultaneous_measurements = len(tau_array)  # 4
