...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        sequence_name = "t1_test" if sequence_name is None else sequence_name
            ...
            with infinite_loop_():
                ...
                with for_(*from_array(i_1, i_1_array)):
                    with for_(*from_array(i_2, i_2_array)):
                        ...
                        align()
                        # Play the mw pulse...
                        play("cw" * amp(Cast.to_fixed(chk_i("MW_puls_power"))), "NV", duration=chk_i("pi_t") * u.ns)
                        align()
                        wait(duration=chk_i("tau") / 4)
                        align()
                        play(
                            pulse="trigit",
                        ...
                        align()
                        play("pulse" * amp(Cast.to_fixed(chk_i("repump_laser_power"))), "Laser_520", duration=readout_len)
                        align()
                        # ... and the laser pulse simultaneously (the laser pulse is delayed by 'laser_delay_1')
                        wait(duration=101 * u.ms - chk_i("tau") / 4)
                        align()
                        play(
                            pulse="trigit",
...
def settings(pdc={}):
    ...
    # tau_array = np.arange(100, 2500000, 5000)
    # tau_array = np.arange(10 * u.us, 50 * u.ms, 500 * u.us)
    tau_array = np.logspace(5, 9, 50) * u.ns
