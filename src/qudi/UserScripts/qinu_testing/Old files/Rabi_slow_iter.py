    ...
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        ...
        with program() as myprog:
            with infinite_loop_():
                ...
                i = declare(int)
                play("pulse" * amp(1), "Laser_520", duration=readout_len * 100 * u.ns)
                for (
                    idx,
                    _I_,
                    ...
                    align()
                    # Play the mw pulse...
                    play("cw" * amp(1), "NV", duration=_I_["tau"] * u.ns)
                    align()
                    wait((3000 - _I_["tau"]) * u.ns)
                    align()
                    # ... and the laser pulse simultaneously (the laser pulse is delayed by 'laser_delay_1')
                    play("pulse" * amp(1), "Laser_520", duration=readout_len * u.ns)
...
def settings(pdc={}):
    ...
    nuclear.number_of_simultaneous_measurements = len(np.arange(10, 2000, 200))  # 4
