...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():  ### #FIXME Achtung... I cant be more than 1 at once...we need to open iterator to for_each loop.
            with program() as myprog:
                a: QuaVariable | QuaProgramArrayVarRefExpression | QuaProgramVarRefExpression = declare(fixed)
                with infinite_loop_():
                    ...
                    play("pulse" * amp(1), "Laser_520", duration=_I_["laser_duration"])
                    align()
                    play(
                        pulse="trigit",
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict(
        (
            ...
            ("laser_duration", np.arange(50, 5000000, 500000)),
            ("readout", ["ssr"]),
            ("repump", ["no", "yes"]),
        )
    )
    nuclear.number_of_simultaneous_measurements = 1  # 4
