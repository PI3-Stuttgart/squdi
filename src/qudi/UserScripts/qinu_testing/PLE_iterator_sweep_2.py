...
from qm import qua
from qua.qm import play, set_dc_offset, declare, fixed, for_, align, amp, infinite_loop_
    ...
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        ...
        def chk_i(key):
            if key == nuclear.sweep_keys[0]:
                return i_1
            if len(nuclear.sweep_keys) == 2:
                if key == nuclear.sweep_keys[1]:
                    return i_2
            else:
            ...
            with infinite_loop_():
                ...
                with for_(*from_array(i_back, nuclear.i_1_array[::-1])):
                    ...
                    with for_(j_back, 0, j_back < j_avg, j_back + 1):
                        play("pulse" * amp(chk_i("repump_laser_power")),"Laser_520", duration=chk_i("backscan_len_pixel")*u.ns / j_avg,)
                    align()
...
def settings(pdc={}):
    ...
    laser_freq_vec_MHz = np.linspace(-7000, -3000, 400)
    nuclear.parameters = OrderedDict(
        (
            ...
            ("click_channel", [2]),
            ("readout_len_pixel", [2 * u.ms]),
            ("backscan_len_pixel", [1 * u.ms]),
        )
    )
