...
readout_len1 = 1 * u.us  # Readout duration for this experiment
readout_len2 = 1 * u.us  # Readout duration for this experiment
readout_len_CRC = 2 * u.ms  # Readout duration for this experiment
...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        ...
        i_1_array = (
            (np.array([self.queue.tt.rp("laser_tuning", omega=i).amp for i in sweeps[0]]) if "MHz" in sweep_keys[0] else sweeps[0])
            if len(sweep_keys) > 0
            else np.array([0])
        )
        i_2_array = (
                ...
                with for_(*from_array(i_1, i_1_array)):
                    with for_(*from_array(i_2, i_2_array)):
                        ...
                        with for_(j, 0, j < j_avg, j + 1):
                            play("pulse" * amp(0.2), "AOM_620", duration=readout_len1 / j_avg)
                        ...
                        ## T1 measurement.
                        align()
                        play("pulse" * amp(0.01), "Laser_520", duration=chk_i("tau"))
                        align()
                        ...
                        with for_(j, 0, j < j_avg, j + 1):
                            play("pulse" * amp(0.2), "AOM_620", duration=readout_len2 / j_avg)
                        ...
                        with for_(j, 0, j < j_avg, j + 1):
                            play("pulse" * amp(0.05), "AOM_620", duration=readout_len_CRC / j_avg)
                            play("pulse" * amp(0.01), "Laser_520", duration=readout_len_CRC / j_avg)
...
def settings(pdc={}):
    ana_seq = [
        ["init", "<", 1, 1, 0, 1],  ### 1: postition of threshold, 2: number of repetitions, 3: distance between threshold, 4: reference measurements
        ["result", ">", 0, 1, 0, 1],  # Put "int(_I_['n_ssr']" as nlp_per_point?
        ["init", ">", 0, 1, 0, 1],
        # ['init', '>', 5, 1, 0, 1],
    ]
    ...
    laser_freq_vec_MHz = np.linspace(1900, 3100, 100)
    nuclear.parameters = OrderedDict(
        (
            ...
            # ("t_read", [100]),
            # ("t_repump", [50]),
            ("tau", np.linspace(1, 100, 100) * u.us),
            ("repump_laser_power", [0.1]),  ###
            ("Laser_freqs_MHz", [2478.0]),
    ...
    nuclear.number_of_simultaneous_measurements = 1  # len(laser_freq_vec_MHz)  # 4
