...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        ...
        i_1_array = sweeps[0] if len(sweep_keys) > 0 else np.array([0])
            ...
            with infinite_loop_():
                ...
                with for_(*from_array(i_1, i_1_array)):
                    with for_(*from_array(i_2, i_2_array)):
                        set_dc_offset("LaserScanner_red", "single", chk_i("Laser_freq_MHz"))
                        ...
                        play(pulse="trigit", element="620_pi", duration=chk_i("aom_pulse_len") / 4)
                        wait(100 * u.us)
                        align()
                        play(
                            pulse="trigit",
...
def settings(pdc={}):
    ...
    laser_freq_vec_MHz = np.linspace(4500, 8500, 400)
