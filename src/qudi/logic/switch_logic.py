...
class SwitchLogic(LogicBase):
    ...
    _watchdog_interval = ConfigOption(
        name="watchdog_interval", default=1.0, missing="nothing"
    )
    _autostart_watchdog = ConfigOption(
    ...
    __wrapped_hw_attributes = frozenset(
        {"switch_names", "number_of_switches", "available_states"}
    )
        ...
        with self._thread_lock:
            try:
                ...
                if self._watchdog_active:
                    ...
                    self.log.exception(
                        f"Error during query of all switch states. "
    ...
    def states(self, state_dict):
        ...
        with self._thread_lock:
            ...
            states = self.states
            if states:
                self.sigSwitchesChanged.emit(
                    {switch: states[switch] for switch in state_dict}
                )
    ...
    def get_state(self, switch):
        ...
        with self._thread_lock:
            try:
                ...
                self.log.exception(
                    f'Error while trying to query state of switch "{switch}".'
                )
                state = None
            return state
