...
class PowerControllerLogic(LogicBase):
    ...
    Demo config:
    power_logic:
        module.Class: 'power_controller_AOM.PowerControllerLogic'
        connect:
    ...
    def on_activate(self):
        ...
        self._rotation_direction.update(
            {ch: 1 for ch in self.channels if ch not in self._rotation_direction.keys()}
        )
        # self._counter = self.counter()
        self._current_positions.update(
            {ch: 0 for ch in self.channels if ch not in self._current_positions.keys()}
        )

        self._power_calibration.update(
            {
                ch: np.array([])
                for ch in self.channels
                if ch not in self._power_calibration.keys()
            }
        )
    ...
    def calibrate_power_wheel(self, motor=1):
        ...
        self.power_calibration[motor] = np.roll(
            calib, np.argmax(calib[:, 1]) - len(calib)
        )
    ...
    def power_calibration(self, calibration):
        ...
        self._power_range.update(
            {
                motor: (min(calibration[motor][:, 1]), max(calibration[motor][:, 1]))
                for motor in calibration.keys()
                if len(calibration[motor]) > 0
            }
        )
    ...
    def set_power(self, power, motor, calibrated):
        ...
        if bool(calibrated) == True:
            if (power > self._power_range[motor][1]) or (
                power < self._power_range[motor][0]
            ):
                print("Power out of calibrated range")
                return -1
            new_angle = self.power_calibration[motor][:, 0][
                np.argmin(np.abs(self.power_calibration[motor][:, 1] - power))
            ]
    ...
    def motor_position(self, step):
        self._motor_pi3.moveRelative(
            motor=self._current_motor,
            pos=step - self._current_positions[self._current_motor],
        )
        delay(3000)
    ...
    def run_saturation(self, motor=0):
        ...
        self._motor_pi3.zeroMotor(motor=motor)
        delay(3000)
        self._saturation_data = np.vstack(
            (self.power_calibration[motor][:, 1], np.array(counts))
        ).T
        self.sig_data_updated.emit()

    def _calculate_corrected_spectrum(self):
        self._saturation_data_corrected = np.copy(self._saturation_data)
        if len(self._saturation_background) == 2 and len(
            self._saturation_background[1, :]
        ) == len(self._saturation_data[1, :]):
            self._saturation_data_corrected[1, :] -= self._saturation_background[1, :]
        else:
            self.log.warning(
                "Background saturation has a different dimension then the acquired saturation. "
