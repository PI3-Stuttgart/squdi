...
def plot_single_values(filename, start_date_str, end_date_str=None, grating="m"):
    ...
    grating_dict = {"h": 1.0 / 60.0 / 60.0, "m": 1.0 / 60.0, "s": 1.0}
    labeldict = {
        "magnet_gradients": ["x", "y"],
        "confocal_pos": ["x", "y", "z"],
        "magnet_pos": ["x", "y", "z", "phi"],
        "current_local_oscillator_freq": "local oscillator",
    }
...
class RabiParametersStatic:
    ...
    def generate_linear_file(self, maxfreq_l):
        ...
        n = 11
        arr = np.array(list(product(*([map(lambda x: "{:.6f}".format(x), E.round_to_amplitude_granularity(np.linspace(0.0, 1.0, n)))] * len(maxfreq_l)))))
    ...
    def check_amplitudes(self, val, **kwargs):
        ...
        if anynan:
            raise Exception(
                "For at least one of given parameters, no valid amplitude could be obtained which resulted in the value NaN. Amplitudes: {}, kwargs: {}".format(
                    val, kwargs
                )
            )
    ...
    def plot(self, **kwargs):
        sample_amp = np.linspace(0.01, 1.0, 100)
        mino = 0.0
...
class Transition:
    ...
    def set_meta(self):
        ...
        if self.spin_type == "14n":
            ...
            self.start_angular_momentum = -0.5
            self.end_angular_momentum = +0.5
...
class TransitionTracker(GenericLogic):
    ...
    si29_list = []  # ['29si5']######          THIS MUST BE LOWERCASE !!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def fit_nuc_transitions(self, trans_list, correct_nuc_trans, use_electron_trans=True):
        def transitions(All=None, Aperp=None, trans_list=[]):
            ...
            if use_electron_trans:
                transit = {i: [-0.5, 0.5] for i in [*self.si29_list, *self.c13_list]}
    ...
    def reload_nuclear_parameters(self):
        ...
        self.g_factors["14n"] = get_last_values_from_file(filename="gamma_14n.dat", full_path=os.path.join(self.log_folder, "gamma_14n.dat"))[
            0
        ]  # r"{}\gamma_14n.dat".format(self.log_folder))[0]
        self.qp = {
            "14n": get_last_values_from_file(filename="qp_14n.dat", full_path=os.path.join(self.log_folder, "qp_14n.dat"))[0]
        }  # r"{}\qp_14n.dat".format(self.log_folder))[0]}
        self.hf_para_n = {
            "14n": get_last_values_from_file(filename="hf_14n.dat", full_path=os.path.join(self.log_folder, "hf_14n.dat"))[0]
        }  # r"{}\hf_14n.dat".format(self.log_folder))[0]}
        self.hf_perp_n = {
            "14n": get_last_values_from_file(filename="hf_perp_14n.dat", full_path=os.path.join(self.log_folder, "hf_perp_14n.dat"))[0]
        }  # r"{}\hf_perp_14n.dat".format(self.log_folder))[0]}
        for key in self.c13_list:
            print(key)
        for key in self.si29_list:
            print(key)
        self.hf_para_n.update(
            dict(
                [
                    (key, get_last_values_from_file(filename="hf_{}.dat".format(key), full_path=os.path.join(self.log_folder, "hf_{}.dat".format(key)))[0])
                    for key in self.c13_list + self.si29_list
                ]
            )
        )
        self.hf_perp_n.update(
            dict(
                [
                    (
                        key,
                        get_last_values_from_file(filename="hf_perp_{}.dat".format(key), full_path=os.path.join(self.log_folder, "hf_perp_{}.dat".format(key)))[
                            0
                        ],
                    )
                    for key in self.c13_list + self.si29_list
    ...
    def set_h_diag(self):
        self.nvham = NVHam(
            magnet_field={"z": self.current_magnetic_field},
            n_type=None,
            nitrogen_levels=[],
            electron_levels=[0, 1, 2, 3],
            gamma=self.g_factors,
            qp=self.qp,
            hf_para_n=self.hf_para_n,
            hf_perp_n=self.hf_perp_n,
        )
        for c13 in self.c13_list:
            self.nvham.add_spin(np.matrix(np.diag([self.hf_perp_n[c13], self.hf_perp_n[c13], self.hf_para_n[c13]])), self.nvham.h_13c(), [0, 1])

        for si29 in self.si29_list:
            self.nvham.add_spin(
                hft=np.matrix(np.diag([self.hf_perp_n[si29], self.hf_perp_n[si29], self.hf_para_n[si29]])), h_ns=self.nvham.h_29si(), nslvl_l=[0, 1]
            )
        ...
        for f in os.listdir(self.log_folder):
            if hasattr(self._awg.mcas_dict, "awgs"):
                ...
                if "e_rabi_ou{:.0f}deg".format(1000 * 0.35) in f:  ###Here it is hardcoded 350 mV of Quantum Machine.
                    ...
                    self.rabi_parameters[f.split(".")[0]] = RabiParametersStatic(filepath=os.path.join(self.log_folder, f), transition_name="e_rabi")
                elif "laser_tuning" in f:
                    self.rabi_parameters[f.split(".")[0]] = RabiParametersStatic(filepath=os.path.join(self.log_folder, f), transition_name="laser_tuning")
    ...
    def mw_mixing_frequency_R(self, val):
        ...
        angle = np.arctan(self.current_magnetic_field_vector[1] / self.current_magnetic_field_vector[0]) * 57.3248  # radian to degrees

        print("Field angle in degrees:", angle)
    ...
    def update_stuff(self):
        for attr_name in [
            "_current_local_oscillator_freq",
            "_mw_mixing_frequency_L",
            "_zero_field_splitting",
            ...
            "ple_A2_fit_params",
            "ple_A1_fit_params",
            "interferometer_fit_params",
            "interferometer_history",
        ]:  # needs "ple_Ex", but where is this "ple_Ex" called?
    ...
    def change_transition_frequency(self, fd, current_magnetic_field=None, test_mode=False):
        ...
        if len(fd14n) > 0:
            ...
            result = mod.fit(
                data=fd14n.values(), x=range(len(fd14n.keys())), params=params, method="powell", fit_kws=dict(options=dict(ftol=1e-17, xtol=1e-17, disp=True))
            )
            ...
            self.update_nuclear_parameter(
                val=result.params["gamma_n"].value, old_val=self.g_factors["14n"], filename="gamma_14n.dat", typ="gyromagnetic ratio", **cd
            )
            self.update_nuclear_parameter(val=result.params["qp"].value, old_val=self.qp["14n"], filename="qp_14n.dat", typ="quadrupole splitting", **cd)
            if len(fd14n) > 2:
                self.update_nuclear_parameter(val=result.params["hf_para_n"].value, old_val=self.hf_para_n["14n"], filename="hf_14n.dat", typ="hyperfine", **cd)
            if len(fd14n) > 3:
                self.update_nuclear_parameter(
                    val=result.params["hf_perp_n"].value, old_val=self.hf_perp_n["14n"], filename="hf_perp_14n.dat", typ="perpendicular hyperfine", **cd
                )
        ...
        for name, new_freq in fdc.items():
            if "13c" in name and "ms0" in name:
                nuc = name[:-4]
                self.update_nuclear_parameter(
                    val=-new_freq / self.current_magnetic_field,
        ...
        for name, new_freq in fdc.items():
            if "13c" in name and not "ms0" in name:
                ...
                nuc = name[:-5]
                self.update_nuclear_parameter(
                    val=ms * (self.g_factors["13c"] * current_magnetic_field + new_freq),
        ...
        for t, target_frequency in fdc.items():
            ...
            logging.getLogger().info(
                "Frequency of {} changed from {} to {} (deviation from target_frequency: {}Hz).".format(
                    t, previous_frequency, current_frequency, (current_frequency - target_frequency) * 1e6
                )
            )
    ...
    @staticmethod
    def frequency_fid(assumed_frequency, frequency_offset, measured_period, max_diff_factor=0.2):
    ...
    def get_rabi_parameter(self, name, **kwargs):
        ...
        if "e_rabi" in name and not "_ou" in name:
            name = "{}_ou{:.0f}deg{}".format(name, 1000 * self._awg.mcas_dict.awgs["2g"].ch[1].output_amplitude, kwargs["mixer_deg"])  # TODO connect to it
    ...
    def rp(self, name, **kwargs):
        if len(kwargs) == 0:
            ...
            return self.rabi_parameters["e_rabi_ou{:.0f}deg{}".format(1000 * self._awg.mcas_dict.awgs["2g"].ch[1].output_amplitude, kwargs["mixer_deg"])]
        else:
    ...
    def current_magnetic_field_vector(self):  # z and x field
        ...
        def odmr(B_z, B_x):
            ...
            return np.array([enerriges[1] - enerriges[0], enerriges[2] - enerriges[1], enerriges[3] - enerriges[2]])
        ...
        return minimize(fun=func, x0=np.array([0.13, 0.0])).x
    ...
    def mfl(self, td, mw_mixing_frequency_L=None, ms_trans="L"):
        ...
        """
        if mw_mixing_frequency_L is None:
            mwfm = self.mw_mixing_frequency_L
