...
class OPX(Base): #hardware, awg,
    ...
    def run_cw_mode(self) -> None:
        curr_do: list = [key for key, value in self.cw_do_states.items() if value == 'on']
        curr_ao = {key: value for key, value in self.cw_ao_values.items() if value != 0.0}
            ...
            with program() as cw_program:
                with infinite_loop_():
                    for ao, power in curr_ao.items():
                        play('power' * amp(self._volt2amp(power)), ao)
