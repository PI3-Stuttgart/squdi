...
class QMDLProTTPLEScanner(ScanningProbeInterface):
    """
    ...
    def _check_scan_end_reached(self):
        ...
        # self._scanned_lines
        return (
            self._time_differences_tasks[0].ready()
            if self._max_rollovers != 0
            else False
        )
    ...
    def _fetch_data_chunk(self):
        try:
            ...
            for num, di_channel in enumerate(self.__active_channels["di_channels"]):
                ...
                idx = self._time_differences_tasks[
                    num
                ].getHistogramIndex()  # current histogram
            ...
            new_data_cum_forward = {
                reverse_routing[key]: samples
                for key, samples in data_td_forward.items()
            }
