...
class DigitalSwitchPS(SwitchInterface):
    ...
    _pulsed_switches = ConfigOption(name='pulsed_switches', default=None, missing='nothing')
    ...
    def on_activate(self):
        ...
        assert isinstance(self._channels, list), 'ConfigOption "channel" must be list type'
        ...
        if state_dict:
            with self.lock:
                ...
                for channel_index, (switch, state) in enumerate(new_states.items()):
                    self.channels_on.append(self._channels[channel_index]) if state is "On" else None
                    state_int = 1 if state is 'On' else 0
                    
                    if switch in self._pulsed_switches:
                        self._pulse_streamer._seq.setDigital(self._channels[channel_index],
                                    [(100000, state_int)])
                        self._pulse_streamer.pulser_on()
                        self._pulse_streamer._seq.setDigital(self._channels[channel_index],
                                    [(100000, int(not bool(state_int)))])
                        # switch_task.write(binary, auto_start=True)
                    
                        # switch_task.write(binary, auto_start=True)
                    else:
                        self._pulse_streamer._seq.setDigital(self._channels[channel_index],
                                    [(100000, int(state_int))])
