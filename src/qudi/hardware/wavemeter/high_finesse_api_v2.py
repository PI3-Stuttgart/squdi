...
class WLM():
    ...
    def set_recording_file(self, filepath: str):
        """
        Sets the file path for the next recording operation.
        Corresponds to SetOperationFile in the DLL. [cite: 1046]
        Args:
            filepath: The full path (string) to the file where data will be recorded.
        Returns:
        ...
        print(f"Setting recording file to: {filepath}")
        encoded_filepath = filepath.encode('utf-8') 
        
        result = self.dll.SetOperationFile(encoded_filepath)
        if result != 0:
            print(f"Warning: set_recording_file for '{filepath}' failed with DLL code {result}.")
        else:
            print(f"Recording file set to '{filepath}' successfully.")
        return result
