...
from qudi.hardware.ni_x_series.helpers import (
    ...
    def on_deactivate(self):
        for channel in list(self._ao_task_handles):
            try:
                ...
                self.log.exception(
                    f'Error while terminating NI analog out task "{channel}":'
                )
    ...
    def _create_ao_task(self, channel: str) -> None:
        ...
        try:
            ...
            ao_task.ao_channels.add_ao_voltage_chan(
                physical_channel=ao_phys_ch, min_val=min_val, max_val=max_val
            )
        except Exception as err:
            try:
                ao_task.close()
