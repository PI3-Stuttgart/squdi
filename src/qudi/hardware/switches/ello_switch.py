...
class DigitalSwitchEllo(SwitchInterface):
    ...
    ello = Connector(interface = 'ThorlabsElloDevices')
    ...
    def on_activate(self):
        ...
        if self._hardware_name is None:
            self._hardware_name = 'Ello ' + (', '.join(map(str, self._channels)))
            ...
            with self.lock:
                ...
                for channel_index, (switch, state) in enumerate(new_states.items()):
                    ...
                    if state == 'On':
                        ...
                        self.ello().ello_rotor.move_rel(45)
                    else:
                        self.ello().ello_rotor.home()
