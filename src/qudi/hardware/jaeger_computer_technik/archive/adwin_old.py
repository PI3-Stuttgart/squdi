...
class adwin(Base, ConfocalScannerInterface):
    ...
    def on_activate(self):
        ...
        # Start process for laser_attenuation control and powermeter flip mount
        self.adw.Start_Process(5)
        self.adw.Start_Process(10)
    ...
    def flip_power_meter(self, on):
        if on:
            self.adw.Set_Par(30, 1)
        if not on:
            self.adw.Set_Par(30, 0)

    def set_digiout(self, port, active):
        if port < 0 or port > 8 or not isinstance(port, int):
            raise Exception('port is not defined')

        par_idx = 8 if port == 0 else port
        if active:
            self.adw.Set_Par(par_idx, 1)
        if not active:
            self.adw.Set_Par(par_idx, 0)


    def change_laser_attenuation(self, green_attenuation):
        self.adw.Set_FPar(30, green_attenuation)
    ...
    def scan_line(self, line_path=None, pixel_clock=False):
        ...
        if pixel_clock==False:
            ...
            self.adw.Set_Par(21, len(line_path[0]))
