...
class Adwin(ScanningProbeInterface):
    ...
    def on_activate(self):
        ...
        self._current_scan_ranges = tuple(
            tuple(rng) for rng in tuple(self._position_ranges.values())[:2]
        )
        self._current_scan_axes = tuple(self._position_ranges)[:2]
        self._current_scan_frequency = max(
            self._frequency_ranges[self._current_scan_axes[0]]
        )
        self._current_scan_resolution = tuple([100] * len(self._current_scan_axes))
        self._current_position = {
            ax: min(rng) + (max(rng) - min(rng)) / 2
            for ax, rng in self._position_ranges.items()
        }
        ...
        channels = [
            ScannerChannel(name="fluorescence", unit="c/s", dtype=np.float64),
        ...
        self.__update_timer.timeout.connect(
            self.get_scan_data, QtCore.Qt.QueuedConnection
        )

        #################################################
        # boot adwin and load processes
        self.boot_adwin()
        self.load_processes()
        #################################################
        return
    ...
    def _randomize_new_spots(self):
        ...
        for x_axis, x_range in self._position_ranges.items():
            for y_axis, y_range in self._position_ranges.items():
                ...
                spot_count = int(
                    round((x_max - x_min) * (y_max - y_min) * self._spot_density)
                )
                ...
                spot_dict["amp"] = np.random.normal(
                    self._spot_amplitude_dist[0],
                    self._spot_amplitude_dist[1],
                    spot_count,
                )
                # spot angle
    ...
    def configure_scan(self, scan_settings):
        ...
        with self._thread_lock:
            ...
            if len(axes) != len(ranges) or len(axes) != len(resolution):
                self.log.error(
                    '"axes", "range" and "resolution" must have same length.'
                )
                return True, self.scan_settings
    ...
    def move_absolute(self, position, velocity=None, blocking=False):
        ...
        with self._thread_lock:
            ...
            if self.module_state() != "idle":
                ...
                self.log.error(
                    "Invalid axes encountered in position dict. Valid axes are: {0}"
                    "".format(set(self._position_ranges))
                )
            else:
                move_distance = {
                    ax: np.abs(pos - self._current_position[ax])
                ...
                if velocity is None:
                    ...
                    move_time = max(
                        0.01,
                        np.sqrt(np.sum(dist**2 for dist in move_distance.values()))
                        / velocity,
                    )
    ...
    def move_relative(self, distance, velocity=None, blocking=False):
        ...
        with self._thread_lock:
            ...
            if self.module_state() != "idle":
                ...
                self.log.error(
                    "Invalid axes encountered in distance dict. Valid axes are: {0}"
                    "".format(set(self._position_ranges))
                )
            else:
                new_pos = {
                    ax: self._current_position[ax] + dist
                    for ax, dist in distance.items()
                }
                if velocity is None:
                    move_time = 0.01
                else:
                    move_time = max(
                        0.01,
                        np.sqrt(np.sum(dist**2 for dist in distance.values()))
                        / velocity,
                    )
                time.sleep(move_time)
    ...
    def get_position(self):
        ...
        with self._thread_lock:
            ...
            position = {
                ax: pos + np.random.normal(0, self._position_accuracy[ax])
                for ax, pos in self._current_position.items()
            }
            return position
        ...
        with self._thread_lock:
            ...
            if len(self._current_scan_axes) == 2:
                y_values = np.linspace(
                    self._current_scan_ranges[1][0],
                    self._current_scan_ranges[1][1],
                    self._current_scan_resolution[1],
                )
            else:
                y_values = np.linspace(
                    self._current_position["y"], self._current_position["y"], 1
                )
            ...
            self._scan_data = ScanData(
                ...
                scan_axes=tuple(
                    self._constraints.axes[ax] for ax in self._current_scan_axes
                ),
    ...
    def get_scan_data(self):
        ...
        with self._thread_lock:
            ...
            if self.module_state() != "idle":
                ...
                line_time = (
                    self._current_scan_resolution[0] / self._current_scan_frequency
                )

                if self._scan_data.scan_dimension == 2:
                    acquired_lines = min(
                        int(np.floor(elapsed / line_time)),
                        self._current_scan_resolution[1],
                    )
                    if acquired_lines > 0:
                        if self.__last_line < acquired_lines - 1:
                            ...
                            for ch in self._constraints.channels:
                                tmp = self._scan_image[
                                    :, self.__last_line : acquired_lines
                                ]
                                self._scan_data.data[ch][
                                    :, self.__last_line : acquired_lines
                                ] = tmp
    ...
    def __start_timer(self):
        if self.thread() is not QtCore.QThread.currentThread():
            QtCore.QMetaObject.invokeMethod(
                self.__update_timer, "start", QtCore.Qt.BlockingQueuedConnection
            )
        else:
    ...
    def __stop_timer(self):
        if self.thread() is not QtCore.QThread.currentThread():
            QtCore.QMetaObject.invokeMethod(
                self.__update_timer, "stop", QtCore.Qt.BlockingQueuedConnection
            )
        else:
            self.__update_timer.stop()
