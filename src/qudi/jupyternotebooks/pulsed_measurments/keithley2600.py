...
class _SMUChannel:
    ...
    def set_current_range(self, value):
        ...
        Examples:
            ...
            you can also use scientific notation: i.e. set the current to 1 ÂµA
    ...
    def set_current(self, value):
        ...
        Examples:
            to set the output level to 10 ÂµA
...
class SMU26xx:
    ...
    def __clear_error_queue(self):
        ...
        """
        # Do a best-effort queue reset during startup.
        # Stale entries (e.g. "Script Aborted by User", -286) may be present from previous sessions.
        # We drain until code==0 without raising here so __init__ can recover.
        cmd = 'errorcode, message = errorqueue.next()\nprint("QERR", errorcode, message)'
        try:
            self.__instrument.write("errorqueue.clear()")
        except Exception:
            pass
        ...
        for _ in range(20):
            ...
            if self.__debug:
                print('Error clear msg: ' + str(response))

            parts = response.split('\t')
            if len(parts) >= 2 and parts[0] == 'QERR':
                try:
                    code_value = float(parts[1])
                except ValueError:
                    continue
                if code_value == 0:
                    return
                # Non-zero code: keep draining.
                continue

            # Non error-queue payload (stale response), clear and continue.
            try:
                self.__instrument.clear()
            except Exception:
                pass
