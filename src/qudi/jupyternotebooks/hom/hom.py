...
def check_ple():
    ...
    for i in range(3):
        ...
        if res.rsquared > 0.5:
            save_ple(tag=f'{elapsed_time}_ple', poi_name='')
            break
        save_ple(tag=f'{elapsed_time}_nodetected', poi_name='')
...
# %%
fw = tagger.write_into_file(os.path.join(folder, 'TEST_g2_II.ttbin'), channels=[2,3, 4, 5, 8, 6])

# Schedule the function
schedule.every(10).seconds.do(check_ple)
