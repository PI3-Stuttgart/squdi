...
class MagnetWindow(GuiBase):
    ...
    def on_activate(self):
        ...
        # Connect buttons for basic magnet control
        ...
        # Logging setup
        _qtpyhandler = QTPyHandler(self._mw.textBrowser_consol)
    ...
    def change_debug_mode(self, debug: bool):
        """Change debug mode and update log levels"""
        if debug:
    ...
    def _update_current_values(self):
        """Update all displayed values and colors based on current magnet state"""
        # Set background color of current values based on ramping state
        ramp_states = self._magnet.get_ramping_state()
        colors = [self.gui_colors.blue] * 3
        
        for i, ramp_state in enumerate(ramp_states):
            if ramp_state == 1:
                colors[i] = self.gui_colors.yellow  # Ramping
            elif ramp_state == 2:
                colors[i] = self.gui_colors.green   # At target
            elif ramp_state == 3:
                colors[i] = self.gui_colors.purple  # Paused
            elif ramp_state == 8:
                colors[i] = self.gui_colors.blue    # At zero
        ...
        # Display measured magnet currents with 2 decimal places
        dec_digits = 2
        ...
        # For target values, show zeros since we don't have access to them
        ...
        # Handle special cases for angles when field is near zero
        if curr_field_spherical[0]*1e3 < 5:  # Less than 5mT
            curr_field_spherical[1] = 0  # theta
            curr_field_spherical[2] = 0  # phi
        elif curr_field_spherical[1] < 0.5 or curr_field_spherical[1] > 179.5:
            # If theta is close to 0° or 180°, phi is undefined
            curr_field_spherical[2] = 0  # phi
            
        # Display current B-field in spherical coordinates
        self._mw.lcdNumber_curr_bfield.display(round(curr_field_spherical[0]*1e3, 0))  # mT
        self._mw.lcdNumber_curr_theta.display(round(curr_field_spherical[1], 1))  # degrees
        self._mw.lcdNumber_curr_phi.display(round(curr_field_spherical[2], 1))  # degrees
    ...
    def start_ramp_pressed(self):
        """Handle ramp button press - start ramping to target values"""
        self.log.debug(f'{__name__}, {inspect.stack()[0][3]}')
        ax0 = self._mw.doubleSpinBox_target_bfield.value() / 1e3  # Convert to T
        ax1 = self._mw.doubleSpinBox_target_theta.value()  # degrees
        ax2 = self._mw.doubleSpinBox_target_phi.value()  # degrees
        params = np.array([ax0, ax1, ax2])
        self.sigRamp.emit(params)
    
    def start_ramp_to_zero_pressed(self):
        """Handle ramp to zero button press"""
        self.log.debug(f'{__name__}, {inspect.stack()[0][3]}')
