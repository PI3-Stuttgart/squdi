...
class SwitchGui(GuiBase):
    ...
    def on_activate(self):
        ...
        self.sigSwitchChanged.connect(
            self.switchlogic().set_state, QtCore.Qt.QueuedConnection
        )
        self._mw.action_periodic_state_check.toggled.connect(
            self.switchlogic().toggle_watchdog, QtCore.Qt.QueuedConnection
        )
        self._mw.switch_view_action_group.triggered.connect(
            self._update_switch_appearance
        )
        self._mw.action_view_highlight_state.triggered.connect(
            self._update_state_colorscheme
        )
        self._mw.action_view_alt_toggle_style.triggered.connect(
            self._update_toggle_switch_style
        )
    ...
    def _populate_switches(self):
        ...
        self._widgets = dict()
        for ii, (switch, states) in enumerate(
            self.switchlogic().available_states.items()
        ):
            ...
            if self._switch_row_num_max is None:
                ...
                grid_pos = [
                    int(ii % self._switch_row_num_max),
                    int(ii / self._switch_row_num_max) * 2,
                ]
            ...
            if len(states) > 2 or self._switch_style == SwitchStyle.RADIO_BUTTON:
                ...
                self._mw.main_layout.addWidget(
                    self._widgets[switch][0], grid_pos[0], grid_pos[1]
                )
                self._mw.main_layout.addWidget(
                    self._widgets[switch][1], grid_pos[0], grid_pos[1] + 1
                )
                switch_widget.sigStateChanged.connect(
                    self.__get_state_update_func(switch)
                )
            elif self._switch_style == SwitchStyle.TOGGLE_SWITCH:
                if self._alt_toggle_switch_style:
                    switch_widget = ToggleSwitchWidget(
                ...
                self._mw.main_layout.addWidget(
                    switch_widget, grid_pos[0], grid_pos[1] + 1
                )
    ...
    def _update_state_colorscheme(self):
        self._state_colorscheme = StateColorScheme(
            self._mw.action_view_highlight_state.isChecked()
        )
